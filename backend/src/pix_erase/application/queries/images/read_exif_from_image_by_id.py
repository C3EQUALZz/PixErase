...
class ReadExifFromImageByIDQueryHandler:
    ...
    async def __call__(self, data: ReadExifFromImageByIDQuery) -> ReadImageExifView:
        ...
        if (
                ...
                and
                typed_image_id not in current_user.images
        ):
            msg = f"Image with id: {data.image_id}, doesnt belong to user with id: {current_user.id}"
        ...
        if image_info_dto.camera is None:
            logger.debug(
                "Image info about camera is None, image id: %s, setting default fields",
                data.image_id
            )
            ...
            logger.debug(
                "Successfully set default camera settings: camera_settings: %s",
                camera_settings
            )
            ...
            if image_info_dto.camera.make is not None:
                make = image_info_dto.camera.make
            else:
                make = UNKNOWN_FIELD

            model: str

            if image_info_dto.camera.model is not None:
                model = image_info_dto.camera.model
            else:
                model = UNKNOWN_FIELD
            ...
            if image_info_dto.camera.focal_length:
                focal_length = image_info_dto.camera.focal_length
            else:
                focal_length = UNKNOWN_FIELD
            ...
            if image_info_dto.camera.aperture_value:
                aperture_value = image_info_dto.camera.aperture_value
            else:
                aperture_value = UNKNOWN_FIELD
        ...
        if image_info_dto.exposure is None:
            ...
            if image_info_dto.exposure.iso is not None:
                iso = image_info_dto.exposure.iso
            else:
                iso = 100

            exposure_bias: str

            if image_info_dto.exposure.exposure_bias:
                exposure_bias = image_info_dto.exposure.exposure_bias
            else:
                exposure_bias = UNKNOWN_FIELD
        ...
        if image_info_dto.flash is None:
            ...
            if image_info_dto.flash.mode:
                mode = FlashModeView[image_info_dto.flash.mode.name]
            else:
                mode = FlashModeView.UNKNOWN
        ...
        if image_info_dto.gps is None:
            ...
            if image_info_dto.gps.latitude is not None:
                latitude = image_info_dto.gps.latitude
            else:
                latitude = 0.0

            longitude: float

            if image_info_dto.gps.longitude is not None:
                longitude = image_info_dto.gps.longitude
            else:
                longitude = 0.0

            altitude: float

            if image_info_dto.gps.altitude is not None:
                altitude = image_info_dto.gps.altitude
            else:
                altitude = 0.0
