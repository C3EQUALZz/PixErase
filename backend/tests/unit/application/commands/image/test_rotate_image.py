...
from pix_erase.application.commands.image.rotate_image import (
    RotateImageCommand,
...
@pytest.mark.asyncio
async def test_rotate_image_schedules_task(
    ...
    expected: TaskID = TaskID("rotate_image:1")
    ...
    # Act
    result = await sut(RotateImageCommand(image_id=image_id, angle=90))
...
@pytest.mark.asyncio
async def test_rotate_image_wrong_owner(
    fake_current_user_service: Mock,
    fake_image_storage: Mock,
    fake_task_scheduler: Mock,
) -> None:
...
@pytest.mark.asyncio
async def test_rotate_image_not_found(
    fake_current_user_service: Mock,
    fake_image_storage: Mock,
    fake_task_scheduler: Mock,
) -> None:
