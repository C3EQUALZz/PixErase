...
from pix_erase.application.commands.image.compress_image import (
...
@pytest.mark.asyncio
async def test_compress_image_schedules_task(
    ...
    user = fake_current_user_service.get_current_user.return_value  # type: ignore[attr-defined]
    ...
    expected: TaskID = TaskID("compress:1")
    ...
    sut = CompressImageCommandHandler(
    ...
    # Act
    result = await sut(CompressImageCommand(image_id=image_id, quality=80))
...
@pytest.mark.asyncio
async def test_compress_image_wrong_owner(
    fake_current_user_service: Mock,
    fake_image_storage: Mock,
    fake_task_scheduler: Mock,
) -> None:
...
@pytest.mark.asyncio
async def test_compress_image_not_found(
    fake_current_user_service: Mock,
    fake_image_storage: Mock,
    fake_task_scheduler: Mock,
) -> None:
