...
@pytest.mark.asyncio
async def test_remove_background_schedules_task(
    ...
    expected: TaskID = TaskID("remove_background_image:1")
    ...
    sut = RemoveBackgroundImageCommandHandler(
    ...
    # Act
    result = await sut(RemoveBackgroundImageCommand(image_id=image_id))
...
@pytest.mark.asyncio
async def test_remove_background_wrong_owner(
    fake_current_user_service: Mock,
    fake_image_storage: Mock,
    fake_task_scheduler: Mock,
) -> None:
...
@pytest.mark.asyncio
async def test_remove_background_not_found(
    fake_current_user_service: Mock,
    fake_image_storage: Mock,
    fake_task_scheduler: Mock,
) -> None:
